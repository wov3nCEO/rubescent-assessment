...
var GLOBAL_FLAGS_BOOLEAN = /* @__PURE__ */ new Set(["--yes", "-y", "--non-interactive"]);
...
function getGlobalFlagsFromArgv(argv) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (GLOBAL_FLAG_NAMES.has(name)) {
      out.push(arg);
      const takesSeparateValue = !GLOBAL_FLAGS_BOOLEAN.has(name) && !arg.includes("=") && i + 1 < args.length && !args[i + 1].startsWith("-");
...
function omitGlobalFlagsFromArgs(args) {
  ...
  for (let i = 0; i < safeArgs.length; i++) {
    ...
    const name = arg.startsWith("--") ? arg.split("=")[0] : arg;
    if (GLOBAL_FLAG_NAMES.has(name)) {
      const skipSeparateValue = !GLOBAL_FLAGS_BOOLEAN.has(name) && !arg.includes("=") && i + 1 < safeArgs.length && !safeArgs[i + 1].startsWith("-");
